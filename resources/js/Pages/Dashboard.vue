...
const fecha=computed(()=>ahora.value.toLocaleDateString('es-CO',{weekday:'long',day:'numeric',month:'long'}))
const hora=computed(()=>ahora.value.toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'}))
    ...
    <template #body><div class="space-y-7 pb-8">
      ...
      <section class="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
        <UCard class="bg-default/85"><template #header><div class="flex items-center justify-between"><div><p class="font-mono text-xs font-bold uppercase tracking-[0.18em] text-primary">Agenda</p><h2 class="mt-1 text-lg font-bold">Próximas citas</h2></div><Link v-if="can('citas.ver')" :href="route('citas.index')"><UButton icon="i-lucide-arrow-up-right" color="neutral" variant="ghost" aria-label="Ver citas"/></Link></div></template>
          <div v-if="proximasCitas.length" class="divide-y divide-default"><Link v-for="cita in proximasCitas" :key="cita.id" :href="route('citas.index')" class="group flex items-center gap-4 py-3 first:pt-0 last:pb-0"><div class="w-14 shrink-0 rounded-lg bg-primary/10 p-2 text-center"><p class="font-mono text-sm font-black text-primary">{{new Date(cita.inicio).toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'})}}</p><p class="text-[10px] uppercase text-muted">{{new Date(cita.inicio).toLocaleDateString('es-CO',{day:'2-digit',month:'short'})}}</p></div><div class="min-w-0 flex-1"><div class="flex items-center gap-2"><p class="font-bold tracking-wider">{{cita.placa}}</p><UBadge size="xs" variant="subtle">{{cita.estado}}</UBadge></div><p class="truncate text-sm">{{cita.vehiculo}}</p><p class="truncate text-xs text-muted">{{cita.servicio||'Servicio por definir'}}</p></div><UIcon name="i-lucide-chevron-right" class="size-4 text-dimmed transition group-hover:translate-x-1 group-hover:text-primary"/></Link></div>
