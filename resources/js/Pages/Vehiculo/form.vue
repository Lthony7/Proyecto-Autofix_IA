...
const state = reactive({
    clienteId: props.vehiculo?.clienteId ?? "",
    placa: props.vehiculo?.placa ?? "",
    marca: props.vehiculo?.marca ?? "",
    modelo: props.vehiculo?.modelo ?? "",
    anio: props.vehiculo?.anio ?? new Date().getFullYear(),
    color: props.vehiculo?.color ?? "",
    kilometraje: props.vehiculo?.kilometraje ?? 0,
    combustible: props.vehiculo?.combustible ?? "gasolina",
...
function guardar() {
    ...
    if (props.vehiculo)
        router.put(
            route("vehiculos.update", props.vehiculo.id),
            state,
            options,
        );
            ...
            >
                    ...
                    ><template #header
                        ><div>
                            ...
                            <p class="text-sm text-muted">
                                Selecciona un cliente activo e identifica el
                                vehículo.
                            </p>
                        </div></template
                    >
                    <div class="grid gap-5 md:grid-cols-2">
                        <UFormField
                            ...
                            ><USelect
                                ...
                                class="w-full"
                                :items="clientes"
                                searchable /></UFormField
                        ><UFormField
                            label="Placa"
                            required
                            :error="errors.placa || errors.placaNormalizada"
                            ><UInput
                                v-model="state.placa"
                                class="w-full uppercase"
                                maxlength="20" /></UFormField
                        ><UFormField
                            label="Marca"
                            required
                            :error="errors.marca"
                ...
                <UCard
                    ...
                    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                        <UFormField label="Año" required :error="errors.anio"
                            ><UInput
                                v-model.number="state.anio"
                                type="number"
                                class="w-full"
                                min="1900" /></UFormField
                        ><UFormField label="Color" :error="errors.color"
                            ><UInput
                                v-model="state.color"
                                class="w-full" /></UFormField
                        ><UFormField
                ...
                >
                    <UButton
                        ...
                        label="Cancelar"
                        :disabled="procesando"
                        @click="router.visit(route('vehiculos.index'))"
                    /><UButton
                        type="submit"
