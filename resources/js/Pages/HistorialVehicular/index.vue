...
const props=defineProps<{vehiculos:Pagina<Vehiculo>;buscar:string}>()
const busqueda=ref(props.buscar)
function filtrar(){router.get(route('historial-vehicular.index'),{buscar:busqueda.value||undefined},{preserveState:true,replace:true})}
</script>

<template>
  <Head title="Historial vehicular" />
  <UDashboardPanel>
    <template #header><UDashboardNavbar title="Historial vehicular"><template #leading><UDashboardSidebarCollapse /></template></UDashboardNavbar></template>
    <template #body>
      <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div><p class="text-sm text-muted">Reparaciones, mantenimientos, diagnósticos y costos por vehículo</p><p class="text-3xl font-semibold">{{vehiculos.total}} vehículos</p></div>
        ...
        <ul v-if="vehiculos.data.length" class="divide-y divide-default overflow-hidden rounded-lg border border-default bg-elevated">
          <li v-for="vehiculo in vehiculos.data" :key="vehiculo.id" class="p-4 sm:p-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <Link :href="route('historial-vehicular.show',vehiculo.id)" class="self-start lg:self-center"><UButton label="Consultar historial" icon="i-lucide-history" size="sm"/></Link>
            </div>
          </li>
        </ul>
