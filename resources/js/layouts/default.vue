...
const esCliente = computed(() =>
    ((page.props.auth as any)?.user?.roles ?? []).includes("Cliente"),
);
const ordenesActivas = computed(() =>
    Number((page.props.navigation as any)?.ordenesActivas ?? 0),
);
...
watch(
    () => page.props.errors,
    (errors) => {
        if (errors && Object.keys(errors as Record<string, string>).length)
            focusInvalidField();
    },
    { deep: true },
);
...
const toggleMenu = (item: { label: string; url: string; active: boolean }) => {
    expandedMenus.has(item.label)
        ? expandedMenus.delete(item.label)
            ...
            ? [
                  {
                      ...
                      url: esCliente.value
                          ? "/mi-historial"
                          : "/historial-vehicular",
                      active: esCliente.value
                          ? page.url.startsWith("/mi-historial")
                          : page.url.startsWith("/historial-vehicular") &&
                            !page.url.startsWith(
                                "/historial-vehicular-bitacora",
                            ),
                      onSelect: () =>
                          navigateTo(
                              esCliente.value
            ...
            ? [
                  {
                      ...
                      onSelect: () =>
                          navigateTo("/historial-vehicular-bitacora"),
                  },
              ]
            : []),
        ...(can("citas.ver")
            ? [
                  {
                      ...
                      url: "/citas",
                      active:
                          page.url.startsWith("/citas") &&
                          !page.url.startsWith("/citas/calendario"),
...
watch(
    () => page.url,
    () => {
        for (const item of links.value[0])
...
<template>
    ...
    >
        ...
        <UDashboardGroup unit="rem">
            <aside
                class="sidebar-shell hidden h-full w-64 shrink-0 flex-col border-r border-default bg-elevated/55 shadow-xl shadow-black/5 backdrop-blur-xl md:flex"
            >
                <div
                    class="border-b border-default bg-gradient-to-b from-primary/5 to-transparent p-3"
                >
                    <TeamsMenu />
                </div>

                <nav
                    class="flex-1 space-y-1.5 overflow-y-auto px-3 py-4"
                ...
                >
                    ...
                    >
                        <p
                            v-if="section.label"
                            class="mb-1.5 px-3 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-dimmed"
                        >
                            {{ section.label }}
                        </p>
                        <div
                        ...
                        >
                            <button
                                ...
                                :class="
                                    item.active || expandedMenus.has(item.label)
                                        ? 'border-primary/20 bg-gradient-to-r from-primary/18 to-primary/5 text-primary shadow-sm shadow-primary/5'
                                        : 'text-muted hover:translate-x-0.5 hover:border-default hover:bg-default/70 hover:text-highlighted hover:shadow-sm'
                                "
                            ...
                            >
                                <span
                                    v-if="
                                        ...
                                        expandedMenus.has(item.label)
                                    "
                                    class="absolute inset-y-2 left-0 w-0.5 rounded-full bg-primary shadow-[0_0_8px_var(--ui-primary)]"
                                />
                                <span
                                    class="grid size-8 shrink-0 place-items-center rounded-lg border transition-colors duration-200"
                                    :class="
                                        item.active ||
                                        expandedMenus.has(item.label)
                                            ? 'border-primary/15 bg-primary/12'
                                            : 'border-transparent bg-elevated group-hover:border-default group-hover:bg-default'
                                    "
                                    ><UIcon
                                        :name="item.icon"
                                        class="size-[18px]"
                                ...
                                <span
                                    class="grid size-7 place-items-center rounded-md bg-default/45"
                                    ><UIcon
                            ...
                            >
                                ...
                                <span
                                    ...
                                    class="min-w-6 rounded-full bg-primary px-1.5 py-0.5 text-center font-mono text-[10px] font-bold leading-4 text-white shadow-sm shadow-primary/20"
                                    >{{ item.badge }}</span
                                >
                            </Link>
                            <Transition name="submenu">
                                <div
                                    v-if="
                                        ...
                                        expandedMenus.has(item.label)
                                    "
                                    class="submenu-tree relative ml-4 mt-1.5 space-y-1 pb-1 pl-7"
                                >
                                    <Link
                                        ...
                                        :class="
                                            page.url.startsWith(child.url)
                                                ...
                                                : 'text-muted hover:translate-x-0.5 hover:border-default hover:bg-default/65 hover:text-highlighted'
                                        "
                                    >
                                        <span
                                            class="tree-branch"
                                        ...
                                        />
                                        <span
                                            class="mr-2 grid size-6 shrink-0 place-items-center rounded-md border transition-all"
                                            :class="
                                                page.url.startsWith(child.url)
                                                    ? 'border-primary/20 bg-primary/15 text-primary shadow-sm'
                                                    : 'border-default bg-elevated text-dimmed group-hover/child:border-primary/15 group-hover/child:text-primary'
                                            "
                                            ><UIcon
                                                :name="child.icon"
                                                class="size-3.5"
            ...
            >
                <template #content>
                    <div
                        ...
                        class="sidebar-shell flex h-full flex-col bg-default/95 backdrop-blur-xl"
                    >
                        <div
                            class="flex items-center justify-between border-b border-default bg-gradient-to-b from-primary/5 to-transparent p-3"
                        >
                            <TeamsMenu />
                            <UButton
                                icon="i-lucide-x"
                                ...
                                >
                                    <button
                                        ...
                                        :class="
                                            item.active ||
                                            expandedMenus.has(item.label)
                                                ? 'border-primary/20 bg-gradient-to-r from-primary/18 to-primary/5 text-primary shadow-sm'
                                                : 'text-muted hover:border-default hover:bg-elevated hover:text-highlighted'
                                        "
                                        :aria-expanded="
                                            expandedMenus.has(item.label)
                                        "
                                    ...
                                    >
                                        ...
                                        <span
                                            class="grid size-8 place-items-center rounded-md bg-default/45"
                                            ><UIcon
                                                name="i-lucide-chevron-right"
                                    ...
                                    <Transition name="submenu">
                                        <div
                                            v-if="
                                                ...
                                                expandedMenus.has(item.label)
                                            "
                                            class="submenu-tree relative ml-5 mt-1.5 space-y-1 pb-1 pl-7"
                                        >
                                            <Link
                                                v-for="child in item.children"
                                                :key="child.url"
                                                ...
                                                :aria-current="
                                                    page.url.startsWith(
                                                        ...
                                                        : undefined
                                                "
                                                class="submenu-link group/child relative flex min-h-10 w-full items-center rounded-lg border border-transparent px-3 py-2 text-left text-sm font-medium transition-all duration-200"
                                                :class="
                                                    page.url.startsWith(
                                                        child.url,
                                                    )
                                                ...
                                                ><span
                                                    ...
                                                    aria-hidden="true"
                                                /><span
                                                    class="mr-2 grid size-7 shrink-0 place-items-center rounded-md border"
                                                    :class="
                                                        page.url.startsWith(
                                                            child.url,
                                                        )
            ...
            <div class="relative flex min-w-0 flex-1 overflow-y-auto overflow-x-hidden">
                ...
                <main
                    id="main-content"
                    tabindex="-1"
                    class="app-content relative z-[2] flex min-w-0 flex-1"
                >
                    <slot />
                </main>
            </div>
...
.submenu-tree::before {
    ...
    background: linear-gradient(
        to bottom,
        color-mix(in srgb, var(--ui-primary) 55%, var(--ui-border)),
        var(--ui-border)
    );
}
...
.tree-branch-active {
    ...
    filter: drop-shadow(
        0 0 3px color-mix(in srgb, var(--ui-primary) 60%, transparent)
    );
}
