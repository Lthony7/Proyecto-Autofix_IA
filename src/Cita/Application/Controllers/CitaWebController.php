...
class CitaWebController extends Controller
{
    ...
    public function create(Request $request): Response
    {
        ...
        return Inertia::render('Cita/form', [
            ...
            'mecanicos' => MecanicoEloquentModel::with(['especialidades' => fn ($q) => $q->wherePivot('activo', true), 'disponibilidades' => fn ($q) => $q->where('activo', true)])->where('estado', 'activo')->orderBy('apellidos')->get()->map(fn ($m) => ['id' => $m->id, 'nombre' => "{$m->nombres} {$m->apellidos}", 'especialidadIds' => $m->especialidades->pluck('id'), 'horarios' => $m->disponibilidades->map(fn ($h) => ['dia' => $h->dia_semana, 'inicio' => substr($h->hora_inicio, 0, 5), 'fin' => substr($h->hora_fin, 0, 5)])]),
            'prefill' => $consultaIa ? ['consultaIaId'=>$consultaIa->id,'clienteId'=>$consultaIa->cliente_id,'vehiculoId'=>$consultaIa->vehiculo_id,'especialidadId'=>$consultaIa->especialidad_sugerida_id,'motivo'=>$consultaIa->entrada['sintoma_principal']??''] : null,
        ]);
    }
    ...
    public function store(GuardarCitaRequest $request, ValidadorDisponibilidad $disponibilidad, RegistrarAuditoria $auditoria): RedirectResponse
    {
        ...
        $cita = DB::transaction(function () use ($datos, $request, $disponibilidad) {
            ...
            if ($consultaIaId) ConsultaIaEloquentModel::whereKey($consultaIaId)->update(['cita_id'=>$cita->id]);
    ...
    public function cambiarEstado(CambiarEstadoCitaRequest $request, CitaEloquentModel $cita, ValidadorDisponibilidad $disponibilidad, RegistrarAuditoria $auditoria): RedirectResponse
    {
        ...
        if (! in_array($nuevo, $transiciones[$cita->estado] ?? [], true)) throw ValidationException::withMessages(['estado' => "No se puede pasar de {$cita->estado} a {$nuevo}."]);

        DB::transaction(function () use ($request, $cita, $nuevo, $disponibilidad) {
            $anterior = ['estado' => $cita->estado, 'inicio' => $cita->inicio->toIso8601String(), 'fin' => $cita->fin->toIso8601String(), 'mecanico_id' => $cita->mecanico_id];
            $cambios = ['estado' => $nuevo, 'actualizado_por' => $request->user()->id];
            if ($nuevo === 'reprogramada') {
                $inicio = CarbonImmutable::parse($request->validated('inicio')); $fin = $inicio->addMinutes($cita->servicio?->duracion_minutos ?? $cita->inicio->diffInMinutes($cita->fin));
                $mecanicoId = $request->validated('mecanico_id') ?: $cita->mecanico_id;
                if ($mecanicoId) { DB::select('select pg_advisory_xact_lock(hashtext(?))', ["cita:{$mecanicoId}:{$inicio->toDateString()}"]); $disponibilidad->validar($mecanicoId, $inicio, $fin, $cita->id); }
                $cambios = [...$cambios, 'inicio' => $inicio, 'fin' => $fin, 'mecanico_id' => $mecanicoId];
            } elseif ($nuevo === 'confirmada' && $request->validated('mecanico_id')) {
                $mecanicoId = $request->validated('mecanico_id'); DB::select('select pg_advisory_xact_lock(hashtext(?))', ["cita:{$mecanicoId}:{$cita->inicio->toDateString()}"]); $disponibilidad->validar($mecanicoId, CarbonImmutable::instance($cita->inicio), CarbonImmutable::instance($cita->fin), $cita->id); $cambios['mecanico_id'] = $mecanicoId;
            ...
            $estadoAnterior = $cita->estado; $cita->update($cambios);
            CitaEstadoHistorialEloquentModel::create(['cita_id' => $cita->id, 'estado_anterior' => $estadoAnterior, 'estado_nuevo' => $nuevo, 'observaciones' => $request->validated('observaciones') ?: $request->validated('motivo'), 'datos_anteriores' => $anterior, 'usuario_id' => $request->user()->id]);
        });
