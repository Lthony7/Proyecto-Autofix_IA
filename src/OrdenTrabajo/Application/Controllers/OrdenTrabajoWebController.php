...
class OrdenTrabajoWebController extends Controller
{
    ...
    public function store(GuardarOrdenRequest $request, RegistrarAuditoria $auditoria): RedirectResponse
    {
        ...
        $auditoria->registrar('orden.creada','orden_trabajo',$orden->id,[],$request);return redirect()->route('ordenes.show',$orden)->with('success','Orden de trabajo creada.');
    }

    public function convertirCita(Request $request,CitaEloquentModel $cita,RegistrarAuditoria $auditoria): RedirectResponse
    {
        abort_unless($request->user()->can('ordenes.crear'),403); if($cita->estado!=='atendida')throw ValidationException::withMessages(['cita'=>'Solo una cita atendida puede convertirse en orden.']);
        $orden=DB::transaction(function()use($cita,$request){$existente=OrdenTrabajoEloquentModel::where('cita_id',$cita->id)->first();if($existente)return $existente;$servicios=$cita->servicio_id?[$cita->servicio_id]:[];if(!$servicios)throw ValidationException::withMessages(['cita'=>'La cita debe tener un servicio antes de crear la orden.']);$mecanicos=$cita->mecanico_id?[$cita->mecanico_id]:[];$orden=$this->crearOrden($cita->cliente_id,$cita->vehiculo_id,$cita->motivo,$cita->kilometraje,$servicios,$mecanicos,$request->user()->id);$orden->update(['cita_id'=>$cita->id]);\Src\AsistenteIA\Infrastructure\Models\ConsultaIaEloquentModel::where('cita_id',$cita->id)->update(['orden_id'=>$orden->id]);return $orden;});
        $auditoria->registrar('cita.convertida_orden','orden_trabajo',$orden->id,['cita_id'=>$cita->id],$request);return redirect()->route('ordenes.show',$orden)->with('success','Orden creada desde la cita.');
    }
    ...
    public function show(Request $request,OrdenTrabajoEloquentModel $orden,CalculadorTotalOrden $calculador): Response
    {
        ...
        return Inertia::render('OrdenTrabajo/show',['orden'=>$this->detalle($orden),'mecanicos'=>MecanicoEloquentModel::where('estado','activo')->orderBy('apellidos')->get()->map(fn($m)=>['label'=>"{$m->nombres} {$m->apellidos}",'value'=>$m->id]),'repuestos'=>$repuestos,'repuestosUsados'=>$usos,'finanzas'=>$finanzas,'pagos'=>$pagos,'factura'=>$factura?['id'=>$factura->id,'numero'=>$factura->numero,'total'=>$factura->total,'emitidaEn'=>$factura->emitida_en->toIso8601String()]:null]);
    }

    public function asignar(AsignarMecanicosRequest $request,OrdenTrabajoEloquentModel $orden,RegistrarAuditoria $auditoria): RedirectResponse
    {
        DB::transaction(function()use($request,$orden){$orden->asignaciones()->where('activo',true)->update(['activo'=>false,'retirado_en'=>now()]);foreach($request->validated('mecanico_ids')as$id)OrdenMecanicoEloquentModel::create(['orden_id'=>$orden->id,'mecanico_id'=>$id,'activo'=>true,'asignado_por'=>$request->user()->id,'observaciones'=>$request->validated('observaciones')]);});
        $auditoria->registrar('orden.mecanicos_asignados','orden_trabajo',$orden->id,['mecanicos'=>$request->validated('mecanico_ids')],$request);return back()->with('success','Mecánicos asignados.');
    }

    public function cambiarEstado(CambiarEstadoOrdenRequest $request,OrdenTrabajoEloquentModel $orden,CalculadorTotalOrden $calculador,RegistrarAuditoria $auditoria): RedirectResponse
    {
        ...
        $auditoria->registrar('orden.estado_cambiado','orden_trabajo',$orden->id,['estado'=>$nuevo],$request);return back()->with('success','Estado de la orden actualizado.');
    }

    public function diagnosticar(RegistrarDiagnosticoRequest $request,OrdenTrabajoEloquentModel $orden,RegistrarAuditoria $auditoria): RedirectResponse
    {
