...
class CrearOrdenDesdeCita
{
    public function crear(CitaEloquentModel $cita, string $usuarioId): OrdenTrabajoEloquentModel
    {
        ...
        if ($cita->servicio) OrdenServicioEloquentModel::create(['orden_id' => $orden->id, 'servicio_id' => $cita->servicio_id, 'nombre_servicio' => $cita->servicio->nombre, 'precio_acordado' => $cita->servicio->precio_base, 'estado' => 'pendiente', 'origen' => 'cita', 'agregado_por' => $usuarioId]);
        foreach ($cita->repuestosSolicitados as $solicitud) OrdenRepuestoRequeridoEloquentModel::create(['orden_id' => $orden->id, 'repuesto_id' => $solicitud->repuesto_id, 'solicitud_cita_id' => $solicitud->id, 'origen' => 'cita', 'descripcion' => $solicitud->descripcion, 'cantidad' => $solicitud->cantidad, 'motivo' => $solicitud->observaciones ?: 'Solicitado por el cliente al agendar.', 'estado' => 'solicitado', 'agregado_por' => $usuarioId]);
        ...
        foreach ($consultas as $consulta) {
            ...
            foreach (($respuesta['repuestos_posibles'] ?? []) as $recomendacion) {
                ...
                $duplicado = OrdenRepuestoRequeridoEloquentModel::where('orden_id', $orden->id)->where('estado', '<>', 'retirado')->where(function ($query) use ($repuesto, $nombre) {
                ...
                OrdenRepuestoRequeridoEloquentModel::create(['orden_id' => $orden->id, 'repuesto_id' => $repuesto?->id, 'origen' => 'ia', 'descripcion' => $nombre, 'cantidad' => $recomendacion['cantidad'] ?? 1, 'motivo' => $motivo, 'estado' => 'sugerido', 'agregado_por' => $usuarioId]);
            }
        }
        return $orden;
    }
