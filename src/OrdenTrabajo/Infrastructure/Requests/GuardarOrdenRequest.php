...
class GuardarOrdenRequest extends FormRequest
{
    ...
    public function rules(): array { return ['cliente_id'=>['required','uuid',Rule::exists('clientes','id')->where('estado','activo')],'vehiculo_id'=>['required','uuid',Rule::exists('vehiculos','id')->where('estado','activo')],'falla_reportada'=>['required','string'],'kilometraje'=>['nullable','integer','min:0','max:9999999'],'servicio_ids'=>['required','array','min:1'],'servicio_ids.*'=>['uuid','distinct',Rule::exists('servicios_taller','id')->where('estado','activo')],'mecanico_ids'=>['array'],'mecanico_ids.*'=>['uuid','distinct',Rule::exists('mecanicos','id')->where('estado','activo')]]; }
